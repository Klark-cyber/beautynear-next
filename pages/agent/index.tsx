...
const AgentList: NextPage = ({ initialInput, ...props }: any) => {
	...
	const likeMemberHandler = async (user: any, id: string) => { //user qaysi agentga like bosmoqchiligini tekshramiz
		try {
			if (!id) return; //agent idsi mavjud bolmasa ishga tushmaydiu
			if (!user._id) throw new Error(Messages.error2); //like bosgan user login bolganini tekshiramiz
			await likeTargetMember({
				variables: { input: id }, // likeTargetMember grphqlga request ketadi
			});


			await getAgentsRefetch({ input: searchFilter }); //Backendga oxirgi ozgarishlarni yuklash uchun sorov yuboramiz.request inputiga searchFilter interfacega javob beradigan inputni path qilamiz.input variablesdan olindi
