...
const Community: NextPage = ({ initialInput, ...props }: T) => {
	...
	const [searchCommunity, setSearchCommunity] = useState<BoardArticlesInquiry>(initialInput); //backendga ketadigan inputning type BoardArticlesInquiry biz uni backnddan chaqirib olganmiz. Dastlabki qiymat sifatida esa pasdagi Initial inputni berdik
	...
	const {
		...
		notifyOnNetworkStatusChange: true, //tinimsiz ozgarishlarni keshga saqlash uchun kerak boladi.yani avtomatik yangilangan malumotlarni keshga olib keladi
		onCompleted: (data: T) => {
	...
	const likeArticleHandler = async (e: any, user: any, id: string) => { //e ni ushlashga sabab agar like yurakcha bosilsa qizaradi. e.stopPropagation(); ni ishlatmasak butun cardni bossak ham yurakcha qizarib qolardi
		try {
			e.stopPropagation();
			if (!id) return;
...
function boardArticlesRefetch(arg0: { input: BoardArticlesInquiry; }) {
