...
const PropertyDetail: NextPage = ({ initialComment, ...props }: any) => {
	...
	const user = useReactiveVar(userVar); //reactive variable ichidagi user malumotlarini user orqali ishlatamiz
	const [propertyId, setPropertyId] = useState<string | null>(null);
	const [property, setProperty] = useState<Property | null>(null);
	const [slideImage, setSlideImage] = useState<string>('');
	const [destinationProperties, setDestinationProperties] = useState<Property[]>([]);
	const [commentInquiry, setCommentInquiry] = useState<CommentsInquiry>(initialComment); //initial comment pasdagi initial commentdan kelyapti
	const [propertyComments, setPropertyComments] = useState<Comment[]>([]); //property commentlarini response ichidan ajratib olamiz.Dastlabki qiymati bosh array boladis
	...
	//Kerakli propertyni tanlab chaqirib olish uchun request
	const {
		loading: getPropertyLoading,      // So'rov ketayotgan vaqtda true bo'ladi
		data: getPropertyData,            // So'rov muvaffaqiyatli bo'lsa, ma'lumotlar shu yerga keladi
		error: getPropertyError,          // Agar xatolik bo'lsa, xato haqida ma'lumot shu yerda bo'ladi
		refetch: getPropertyRefetch,      // So'rovni qaytadan yuborish uchun ishlatiladigan funksiya
	} = useQuery(GET_PROPERTY, {        // GET_PROPERTIES - bu sizning GraphQL so'rov (query)ingiz
		fetchPolicy: 'network-only',   // Birinchi keshdan ko'rsatadi, keyin tarmoqdan yangi ma'lumotni olib keshni yangilaydi 
		variables: { input: propertyId }, // So'rovga yuboriladigan parametrlar (initialInput props-dan kelmoqda)
		skip: !propertyId, //yani agar router(url query) ichida propertyId bolmasa backendga request qilma mantigi
		...
		onCompleted: (data: T) => {         // So'rov muvaffaqiyatli yakunlanganda ishga tushadigan callback funksiya
			if (data?.getProperty) setProperty(data.getProperty) // agar data uani property mavjud bolsa unga tegishli malumotlarni property ichiga joylab oldik
			if (data?.getProperty) setSlideImage(data.getProperty?.propertyImages[0]); //property imageni ushlab oldik
		},
	});

	//Kerakli property joylashgan hududdagi boshqa propertylarni variant sifatida korsatish 
	const {
		...
		variables: {
			input: {
				page: 1,
				limit: 4,
				sort: "createdAt", //yani osha lokatsiya boyicha dbdagi eng songi qoshilgan propertylar keladi
		...
		skip: !propertyId && !property, //Agar urlda tanlangan property mavjud bolmasa backendga sorov yuborma
		notifyOnNetworkStatusChange: true,  // So'rov qayta yuborilganda (refetch) loading holati o'zgarishini kuzatib boradi
		onCompleted: (data: T) => {         // So'rov muvaffaqiyatli yakunlanganda ishga tushadigan callback funksiya
			if (data?.getProperties?.list)
				setDestinationProperties(data?.getProperties?.list); // Kelgan propertylarni destinationProperties ozgaruvchisiga saqlab oldik
		},
	});

	const {
		loading: getCommentsLoading,      // So'rov ketayotgan vaqtda true bo'ladi
		data: getCommentsData,            // So'rov muvaffaqiyatli bo'lsa, ma'lumotlar shu yerga keladi
		error: getCommentsError,          // Agar xatolik bo'lsa, xato haqida ma'lumot shu yerda bo'ladi
		refetch: getCommentsRefetch,      // So'rovni qaytadan yuborish uchun ishlatiladigan funksiya
	} = useQuery(GET_COMMENTS, {        // GET_PROPERTIES - bu sizning GraphQL so'rov (query)ingiz
		fetchPolicy: 'cache-and-network',   // Birinchi keshdan ko'rsatadi, keyin tarmoqdan yangi ma'lumotni olib keshni yangilaydi 
		variables: { input: initialComment },
		skip: !commentInquiry.search.commentRefId, //Agar commentInquiry ichidagi search ichida propertyning idsi mavjud bolmasa sorov amalga oshirilmaydi
		notifyOnNetworkStatusChange: true,  // So'rov qayta yuborilganda (refetch) loading holati o'zgarishini kuzatib boradi
		onCompleted: (data: T) => {         // So'rov muvaffaqiyatli yakunlanganda ishga tushadigan callback funksiya
			if (data?.getComments?.list)
				setPropertyComments(data?.getComments?.list); // Kelgan propertylarni destinationProperties ozgaruvchisiga saqlab oldik
			setCommentTotal(data?.getComments?.metaCounter[0]?.total ?? 0); //kommentlarning umumiy sonini ajratib olamiz
		},
	});

	/** LIFECYCLES **/
	useEffect(() => { //urldagi query ichidagi property idsi mavjud bolsa ushbu useEffect ishg atushadi
		if (router.query.id) {
			setPropertyId(router.query.id as string);
			setCommentInquiry({
				...commentInquiry, //spread orqali commentInquiry ning dastlabki initialInquirysini yangilaymoiz.Natijada commentInquiry yangilanib backendga osha propertyga oid commentlarni olish uchun sorov ketadi
				search: {
					commentRefId: router.query.id as string, //urldagi property idsini commentRefId ga tenglaymiz
				},
			});
	...
	/** HANDLERS **/
	const changeImageHandler = (image: string) => {   // Propertyga tegishli rasmlarni ustiga bosilganda osha rasm katta korinishda korinadi
		setSlideImage(image);
	};
	...
	const likePropertyHandler = async (user: T, id: string) => {
		try {
			...
			await getPropertyRefetch({
				input: id //yani detail ichidagi kerakli propertyga like bosilganda get property yangilanyapti.Buning uchun property idsini input sifatida path qildik
			})

			// Ro'yxatni yangilash (refetch) uchun so'rov yuborish.Yani variant sifatida taqdim qilingan propertylarni qayta yangilaymiz natijada like bosilgan propertylar yurakchasi qizaradi
			...
			}); //input backend kutayotgan input kalit sozi, initialInput yangilanishi kerak bolgan data.user like bossa yoki likeni qaytarib olsa backenddagi data ozgaradi shu ozgargan datat darxol namoyon boladi
	...
	const commentPaginationChangeHandler = async (event: ChangeEvent<unknown>, value: number) => { //commentlar uchun paginateon hosil qilish uchun handler
	...
	const createCommentHandler = async () => {
		try {
			...
			setInsertCommentData({ ...insertCommentData, commentContent: '' }) // setInsertCommentData orqali insertCommentDatani yangiladik va commentContent ni bosh string qildik keyingi commentlar uchun maydonni boshatdik
			await getCommentsRefetch({ input: commentInquiry }) //Yuqorida yangi comment hosil qildik endi uni pageni refresh qilib backenddan kelgan data asosida yangilab olamiz
		} catch (err: any) {
			await sweetErrorHandling(err);
		}
	}

	if (getPropertyLoading) { //Agar GET_PROPERTY natijasida hali data kelishga ulgurmagan bolsa loading jarayonida amalga oshadigan mantiq
			...
			<div id={'property-detail-page'}>
				<div className={'container'}>
					<Stack className={'property-detail-config'}>
										...
										<Stack className="button-box">
											{property?.meLiked && property?.meLiked[0]?.myFavorite ? (
												...
												<FavoriteBorderIcon
													fontSize={'medium'}
													// @ts-ignore
													onClick={() => likePropertyHandler(user, property?._id)} //propertyga like bosish mantigi
												/>
											)}
								...
								{commentTotal !== 0 && (
									<Stack className={'reviews-config'}>
										...
										<Stack className={'review-list'}>
											{propertyComments?.map((comment: Comment) => { // propertyComments orqali propertyga oid commentlarni Review commmand component ichiga props qilyapmiz
								...
								<Stack className={'leave-review-config'}>
									...
									<Box className={'submit-btn'} component={'div'}>
										<Button
											...
											onClick={createCommentHandler} //button bosilganda comment create boladi va yangi sorov yuborilib comment darhol pageda korinadi
										>
						...
						{destinationProperties.length !== 0 && ( //Agar tanlangan property joylashgan hududda boshqa propertylar ham mavjud bolsa ularni variant sifatida taqdim qilamiz
										...
										{destinationProperties.map((property: Property) => {
											return (
												<SwiperSlide className={'similar-homes-slide'} key={property.propertyTitle}>
													<PropertyBigCard property={property} key={property?._id} likePropertyHandler={likePropertyHandler} /> {/** PropertyBigCard ichiga kerakli propertyni props qilamiz*/}
												</SwiperSlide>
											);
										})}
									</Swiper>
