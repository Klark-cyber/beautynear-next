...
import PropertyCard from '../salon/Saloncard';
...
const MyFavorites: NextPage = () => {
	...
	const likePropertyHandler = async (user: T, id: string) => {
		try {
			...
			if (!user._id) throw new Error(Messages.error2); // Foydalanuvchi ID-si bo'lmasa, xato otish
			...
			await getFavoritesRefetch({ input: searchFavorites }); //input backend kutayotgan input kalit sozi, initialInput yangilanishi kerak bolgan data.user like bossa yoki likeni qaytarib olsa backenddagi data ozgaradi shu ozgargan datat darxol namoyon boladi
		} catch (err: any) {
