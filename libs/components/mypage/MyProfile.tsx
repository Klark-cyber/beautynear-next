...
const MyProfile: NextPage = ({ initialValues, ...props }: any) => {
	...
	const updateMemberHandler = useCallback(async () => { //useCallback asosida ishga tushadigan handler bolib updateData har ozgarganda ishga tushadi
		try {
			if (!user._id) throw new Error(Messages.error2); //user login bolganini tekshiramiz
			updateData._id = user._id;
			const result = await updateMember({
				variables: {
					input: updateData, //request inputiga initialValues ni kiritamiz
				},
			});

			//Frontenddagi datalrni ham yangilab olamiz.
			// @ts-ignore
			const jwtToken = result.data.updateMember?.accessToken; //tokenni yangilaymiz.token yangilanganda usevar + localstorage yangilanadi
