...
import { PropertyLocation, PropertyType } from '../../enums/salon.enum';
import { REACT_APP_API_URL, propertySquare } from '../../config';
import { PropertyInput } from '../../types/service/service.input';
...
const AddProperty = ({ initialValues, ...props }: any) => {
	...
	const [insertPropertyData, setInsertPropertyData] = useState<PropertyInput>(initialValues); //backemddan keladigan data uchun initial inputni hosil qilib olamiz
	...
	/** APOLLO REQUESTS **/
	const [createProperty] = useMutation(CREATE_PROPERTY); //yangi property hosil qilishda ishlatiladi
	const [updateProperty] = useMutation(UPDATE_PROPERTY); //eski propertyni update qilishda ishlatiladi

	const {
		loading: getPropertyLoading, //bu true/false qiymatni qaytaradi
		...
		variables: {
			input: router.query.propertyId,  //agar queryda id bolsa demak bu update uchun xizmat qiladi.
		},
	});

	/** LIFECYCLES **/
	useEffect(() => { //Backenddan klgan data asosida InsertPropertyDatani qiymatlarini bosh holatdan kerakli qiymatlar asosida shakllantirib olamiz.Shuningdek bu agar property yangi qoshiladigan bolsa : dan keyingi keladigan qismni dastlabki qiymat sifatida vaqtincha oladi
		setInsertPropertyData({
			...insertPropertyData, //dastlabki qiymatni spread qilish xavfsizlik uchun kerak.Shundoq ham backenddan toliq malumot kelar edi
	...
	}, [getPropertyLoading, getPropertyData]); //1 renderda useeffect ishga tushishi tabiiy ammo 2 renderda getPropertyLoading(true) => falsega ozgaradi (yani loadiung yakunlanib data kirib kelgach ishora falsega ozgaradi). 2-data kelgach getPropertyData ning qiymati "" dan kerakli qiymatga ozgaradi shu sababli useEffect yana render boladi.
	/**
	Component render
		↓
	initialValues statega tushadi
		↓
	useQuery backendga request yuboradi
		↓
	backenddan data keladi
		↓
	getPropertyData o‘zgaradi
		↓
	useEffect ishlaydi
		↓
	setInsertPropertyData()
	...
	state yangilanadi
		↓
	component qayta render bo‘ladi
		↓
	inputlar backenddagi qiymatlar bilan to‘ladi
	 */
	...
	async function uploadImages() {
		try {
			...
			const selectedFiles = inputRef.current.files; //inputRef bu useRef orqali hosil bolgan varieble. u inputdagi formdan current filesni qabul qiladi

			if (selectedFiles.length == 0) return false; //kamida 1 file 
			if (selectedFiles.length > 5) throw new Error('Cannot upload more than 5 images!');

			formData.append(
				'operations', //komandani operations nomi bilan formdataga yuklayapmiz
				JSON.stringify({
					query: `mutation ImagesUploader($files: [Upload!]!, $target: String!) {  
						imagesUploader(files: $files, target: $target) 
				  }`, //query komanda esa yuqoridagini tashkil qiladi
					variables: {
						files: [null, null, null, null, null], //dastlabki qiymat null
						target: 'property', //target qiymati property.
					},
				}),
			);
			formData.append(
			...
			const response = await axios.post(`${process.env.REACT_APP_API_GRAPHQL_URL}`, formData, { //Backendga response qilishda axiosdan foydalanamiz
				headers: {
					'Content-Type': 'multipart/form-data',
					'apollo-require-preflight': true,
					Authorization: `Bearer ${token}`, //authorethationda Bearer va token nomi ostida user nomini yuboramiz.
				},
			});
	...
	const insertPropertyHandler = useCallback(async () => { //yangi property hosil qilish uchun handler
		try {
			const result = await createProperty({ // CREATE_PROPERTY ga sorov yuboramiz input sifatida insertPropertyData ni path qilamiz 
				variables: {
			...
			await router.push({ //create muvafaqqiyatli yakunlansa /mypage ga yonaltirib myProperties qismga yuboramiz
				pathname: '/mypage',
				query: {
	...
	const updatePropertyHandler = useCallback(async () => { //propertyni update qilish uchun handler
		try {
			// @ts-ignore
					...
					<Stack className="config">
						...
						<Stack className="buttons-row">
							{router.query.propertyId ? (
								<Button className="next-button" disabled={doDisabledCheck()} onClick={updatePropertyHandler}> {/**Agar queryda id bolmasa updatePropertyHandler ishga tushadi  */}
									<Typography className="next-button-text">Save</Typography>
								</Button>
							) : (
								<Button className="next-button" disabled={doDisabledCheck()} onClick={insertPropertyHandler}> {/**Agar queryda id bolmasa insertPropertyHandler ishga tushadi  */}
									<Typography className="next-button-text">Save</Typography>
								</Button>
							)}
						</Stack>
