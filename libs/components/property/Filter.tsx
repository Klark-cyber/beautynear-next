...
const Filter = (props: FilterType) => {
	...
	useEffect(() => {
		...
		if (searchFilter?.search?.locationList?.length == 0) { //agar location list bosh bolsa yani user lokatsiyani tanlamasa ishga tushuvchi mantiq
	...
	/** HANDLERS **/
	const propertyLocationSelectHandler = useCallback( //useCallback bu propertyLocationSelectHandler ichida ishga tushuvchi mantiq vau 131 qatordagi searchFilter yangilanganda ishga tushadi
		async (e: any) => {
			try {
				const isChecked = e.target.checked;
				const value = e.target.value;
				if (isChecked) {
					await router.push( //agar kerakli lokation belgilansa uni router objecti ichiga joylayapmiz
						`/property?input=${JSON.stringify({
							...searchFilter,
							...
							return (
								<Stack className={'input-box'} key={location}>
									<Checkbox
										...
										size="small"
										value={location}
										checked={(searchFilter?.search?.locationList || []).includes(location as PropertyLocation)} //agar urldagi input params ichida itirate bolayotgan location mavjud bolsa checkboxni checked qil mantigi
